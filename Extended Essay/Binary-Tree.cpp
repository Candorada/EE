...
void insertBalanced(Node* root, int low, int high,int& items) { // too untrustworthy
    if (low > high) return;
    int mid = low + (high - low) / 2;
    Node* newNode = root->appendNode(items*(items%2==0?1:-1));
    ...
    insertBalanced(root, low, mid - 1,items);
    insertBalanced(root, mid + 1, high,items);
}
int main(){
    ...
    for(int i=0;i<1000;i++){ //insert 1 million randm numbers
